--[[As of 6/6/2022, Obelus X bypasses all public anti-cheats and is universal, if it is patched anywhere, dm the creator of this script for it to be bypassed at that experience.]]--
--[[local Settings = getgenv().Settings or {
    Enabled = false, -- If the reach is enabled or not.
    Visible = false, -- If the reach is visible or not.
    Size = Vector3.new(7, 7, 7), -- Size of the reach.
    
    Shape = "Ball", -- Shape of the reach. {Block, Ball, Wall, Cylinder}
    Position = "Sword", -- Reach position. {Sword, HumanoidRootPart}
    Chroma = Color3.fromRGB(0, 0, 0), -- Color of the reach. Set this to Rainbow instead of Color3 for Rainbow chroma.
    
    TeamChecks = false, -- Checks if enemy is on the same team or not.
    VisibleChecks = false, -- Checks if enemy is visible or not.
    HealthChecks = false, -- Checks if enemy is alive to damage, or if their health is above 100 it won't damage.
    
    HitClock = 0.25, -- Higher this clock is, more chance it will reach the player.
    HitDelay = 0, -- Higher this is, the more delayed the reach will be, 0 is the minimum, 1 is the maximum.
    DamageAmp = false, -- Determines whether or not damage-amp is enabled.
    DamageAmpSettings = {
        ThreadAmount = 10, -- Amount of threads for damage-amp.
        DamageDistanceBased = false, -- If the damage is distance-based or not. -- (Deprecated)
        MaximumDamageDistance = math.huge, -- The maximum amount of studs it will reach with damage.
        HvHMode = false -- Makes damage-amp way faster, plus implementing the damage-settings you chose above. This is good for cheating against cheaters and doing insane damage. (Deprecated)
    },

    ReachOnlyOnLunge = false -- If set to true, will only reach when the sword is lunging.
}
...
local Keys = getgenv().Keys or {
    ...
    ShapeCycleKey = "V",
    AutoClickerKey = "Q" -- (Deprecated)
}

local Experimental = getgenv().Experimental or {
    LockdownMode = false, -- Attempts to remove all hooks and stuff that would get caught at anti-cheats.
    FakeHandleChecks = false, -- Attempts to spoof handle and create fake implementation objects to try and help bypass some anti-cheats.
    NotificationsEnabled = true -- If enabled, then notifications will show up and warnings will show up in console, if disabled, they will not show up in console or as a notification.
}--]]
